...
object Util {
  def model1 = {
    ...
    val tparam = Type.Param(Seq(Covariant), Type.Name("A"), Seq(),
      ...
      , Seq(), Seq());
    val fooMethod = Def(Name("foo", 1), Type.Name("String"), Seq(Seq(Term.Param("a", Seq(), Type.Name("String")))), Seq(), Comment("Cool method!"), Seq(Abstract))
